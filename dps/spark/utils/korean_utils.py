...
SPAM_REMOVE = [
    ...
    (
        # * [Seonghee] 판례에서 예로 들어 "주문과 같이 판결한다." 이후의 모든 텍스트 제거
        re.compile(r"주문과\s*같이\s*판결\s*한다\.\s*.*"),
        r"주문과 같이 판결한다."
    ),
    (
        # 기타 판결 텍스트 포맷 처리
        re.compile(r"(주문과\s*같이\s*결정한다\.).*"),
        r"주문과 같이 결정한다."
    ),
    (
        re.compile(r"(주문에\s*쓴\s*바와\s*같이\s*판결한다\.).*"),
        r"주문에 쓴 바와 같이 판결한다."
    ),
    (
        re.compile(r"(주문과\s*같이\s*형을\s*정한다\.).*"),
        r"주문과 같이 형을 정한다."
    ),
    (
        re.compile(r"(판결의\s*요지를\s*공시한다\.).*"),
        r"판결의 요지를 공시한다."
    ),
    (
        re.compile(r"(주문을\s*선고해야\s*한다\.).*"),
        r"주문을 선고해야 한다."
    ),
    (
        re.compile(r"(관여법관들의\s*견해가\s*일치되다\.).*"),
        r"관여법관들의 견해가 일치되다."
    ),
]
